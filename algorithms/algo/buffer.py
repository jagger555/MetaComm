...
class Trajectory:
    ...
    def __init__(self, **kwargs):
        ...
        """
        missing = [name for name in self.BASE_NAMES if name not in kwargs]
        if missing:
            raise KeyError(f"Missing trajectory fields: {missing}")
        self.extra_names = [name for name in kwargs.keys() if name not in self.BASE_NAMES]
        self.names = self.BASE_NAMES + self.extra_names
    ...
    @classmethod
    def base_names(cls):
        return list(cls.BASE_NAMES)
    
    @classmethod
    def names(cls):
        return cls.base_names()
...
class TrajectoryBuffer:
    ...
    def store(self, s, a, r, s1, d, logp, **kwargs):
        """
        ...
        for name, item in kwargs.items():
            ...
            item = item.view(b, n, -1)
            self.extra.setdefault(name, []).append(item)
    ...
    def retrieve(self):
        ...
        返回n_thread条traj
        """
        names = Trajectory.base_names() + list(self.extra.keys())
        trajs = []
        traj_all = {}
        if self.s == []:
            return []
        for name in Trajectory.base_names():
        ...
        for name, values in self.extra.items():
            traj_all[name] = torch.stack(values, dim=1)
        n = traj_all['s'].size()[0]
        for i in range(n):
            traj_dict = {}
            for name in names:
