...
class DPPOAgent(nn.ModuleList, YyxAgentBase):
    ...
    def __init__(self, logger, device, agent_args, input_args):
        ...
        # Only communication-based agents define a separate collaboration lr.
        self.lr_colla = getattr(agent_args, 'lr_colla', None)
        ...
        # 计算 action_dim
        if hasattr(agent_args, "action_dim"):
            self.action_dim = agent_args.action_dim
        else:
            # 如果 action_space 是 gym.Space 的列表
            if isinstance(self.action_space, list):
                self.action_dim = sum([dim.n for dim in self.action_space])
            else:
                # 直接取离散动作空间维度
                self.action_dim = self.action_space.n if hasattr(self.action_space, "n") else self.action_space
        ...
        if input_args.algo not in ('G2ANet', 'G2ANet2', 'IPPO', 'MetaComm', 'CommG2ANet', 'UCSMAPPO'):
    ...
    def get_networked_s(self, s, which_net, phase=None):
        ...
        if self.input_args.algo in ('G2ANet', 'G2ANet_CommFormer'):  # 拿到hard_att的邻居图后，根据邻居图对obs取并集
    ...
    # def s_for_agent(self, s, i):
        """
        返回第 i 个 agent 的观测，兼容以下输入类型：
          - tensor: shape [batch, n_agent, dim] 或 [n_thread, n_agent, dim]
          - list/tuple: 每个元素可能是单 agent 的 tensor，或为邻居拼接后的 tensor（batch, degree*obs_dim）
        若遇到 degree*obs_dim，会 reshape 为 (batch, degree, obs_dim) 并对 degree 维取 max，
        最终返回 (batch, obs_dim) 的 tensor（放到 self.device 并 contiguous()）。
        """
        # list/tuple（如 G2ANet 分支返回的 ans_s）
        if isinstance(s, (list, tuple)):
            ...
            # 如果是拼接形式 (batch, degree * obs_dim)，把它 reshape 成 (batch, degree, obs_dim)
            if out.dim() == 2 and out.size(1) % self.observation_dim == 0 and out.size(1) != self.observation_dim:
                degree = out.size(1) // self.observation_dim
                # reshape 并对 degree 维取 max（也可改为 mean/sum，按设计需求）
                out = out.view(out.size(0), degree, self.observation_dim)
                out = torch.max(out, dim=1)[0]  # (batch, obs_dim)
            return out.contiguous()

        # 如果是 tensor（常见情况）
        if torch.is_tensor(s):
            if s.dim() == 3:
                return s[:, i, :].to(self.device).contiguous()
            if s.dim() == 2:
                return s[i].unsqueeze(0).to(self.device).contiguous()
            if s.dim() == 1:
                return s.unsqueeze(0).to(self.device).contiguous()

        raise ValueError(f"Unsupported observation type/shape in s_for_agent: {type(s)} {getattr(s, 'shape', None)}")
    ...
    def act(self, s, phase):
        ...
        with torch.no_grad():
            ...
            for i in range(self.n_agent):
                print("Input shape to network:", self.s_for_agent(s, i).shape)
        ...
        for i_update in range(self.n_update_pi):
            ...
            for i_pi in range(1):
                ...
                if self.input_args.debug:
                    if self.input_args.algo == 'G2ANet':  # 新增条件判断
