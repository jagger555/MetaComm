...
def parallelEval(agents, func, args):
    ...
    """
    if ray is None:
        return sequentialEval(agents, func, args)
    results = []
...
class LogServer(object):
    ...
    def __init__(self, args):
        ...
        self.scalar_csv_path = os.path.join(run_args.output_dir, 'scalars.csv')
        if not os.path.exists(self.scalar_csv_path):
            with open(self.scalar_csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
    ...
    def flush(self, logger=None):
        ...
        for key in buffer:
            ...
            if isinstance(data[log_key], torch.Tensor) and len(data[log_key].shape) > 0 or \
                ...
                scalar_value = data[log_key]
                if isinstance(scalar_value, torch.Tensor):
                    scalar_value = scalar_value.item()
                elif isinstance(scalar_value, np.ndarray):
                    scalar_value = scalar_value.item()
                elif isinstance(scalar_value, np.generic):
                    scalar_value = scalar_value.item()
                self.writer.add_scalar(log_key, scalar_value, self.step*self.input_args.n_thread)
                csv_rows.append((self.step*self.input_args.n_thread, log_key, scalar_value))
        if data:
            self.writer.flush()
        if csv_rows:
