...
class GuardSubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        obs = [torch.as_tensor(item) for item in obs]
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        obs = [torch.as_tensor(item) for item in obs]
        return torch.stack(obs), np.stack(rewards), np.stack(dones), infos
    ...
    def get_obs_from_outside(self):
        ...
        results = [remote.recv() for remote in self.remotes]
        results = [torch.as_tensor(item) for item in results]
