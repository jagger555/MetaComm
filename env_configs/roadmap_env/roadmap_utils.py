...
def adj_to_timestamped_geojson(adjs, current_times, adj_coords, uav_trajs, input_args, env_config):
    # 检查 uav_trajs 的长度
    if len(uav_trajs) < 2:
        raise ValueError(f"uav_trajs length ({len(uav_trajs)}) is less than 2")
    ...
    for t, adj in enumerate(adjs):
        ...
        # 动态生成 UAV 之间的连线
        for i in range(len(uav_trajs)):
            for j in range(i + 1, len(uav_trajs)):
                # 检查索引是否越界
                if len(uav_trajs[i]) <= t or len(uav_trajs[j]) <= t:
                    raise ValueError(f"uav_trajs[{i}] or uav_trajs[{j}] length is insufficient for t={t}")
                ...
                feature = {
                    ...
                    "geometry": {
                        "type": "LineString",
                        "coordinates": (list(uav_trajs[i][t+1]), list(uav_trajs[j][t+1])),
                    },
                    "properties": {
                        ...
                        "style": {
                            "color": color,
                            "opacity": opacity,
                            'weight': 5 if int(adj[i][j] + adj[j][i]) > 0 else 0,
                        },
                        "code": 11,
                    },
                }
