...
proj_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'source_code')
print('---------------------')
print(proj_dir)
sys.path.append(proj_dir)
os.chdir(proj_dir)  # 设置工作路径为 source_code
...
def getRunArgs(input_args):
    ...
    run_args.seed = input_args.seed
    return run_args


def getAlgArgs(run_args, input_args, env):
    assert input_args.env in ('Mobile', 'UCS') or input_args.env.startswith('Mobile')
    assert input_args.algo in ['DPPO', 'CPPO', 'IPPO', 'DMPO','CommG2ANet',
                               'IC3Net', 'IA2C', 'G2ANet', 'G2ANet2', 'ConvLSTM', 'GCRL', 'Random', 'MetaComm', 'CommNet', 'UCSMAPPO']
...
def override(alg_args, run_args, input_args, env, agent_fn):
    if input_args.use_snrmap and hasattr(env, 'cell_num'):
    ...
    if run_args.seed is None:
        from get_args import resolve_seed
    ...
    if getattr(input_args, 'reward_pref', None) is not None:
        run_args.name += f'_RewardPref={input_args.reward_pref:g}'
    ...
    if input_args.tau_schedule == 'fixed':
        if input_args.tau != 1.0:
            run_args.name += f'_TAU={input_args.tau}'
    else:
        run_args.name += (
            f'_TauSched={input_args.tau_schedule}'
            f'_TauStart={input_args.tau_start}'
            f'_TauEnd={input_args.tau_end}'
        )
        if input_args.tau_anneal_end_iter > 0:
            run_args.name += f'_TauAnnealIter={input_args.tau_anneal_end_iter}'
    ...
    run_args.name += f'_Seed={run_args.seed}'

    # Autoregressive ordering ablation tagging
    if getattr(input_args, 'agent_order_mode', 'fixed') != 'fixed':
        run_args.name += f'_Order={input_args.agent_order_mode}'
        if input_args.agent_order_mode == 'custom':
            run_args.name += f'_OrderSeed={input_args.agent_order_seed}'

    # Apply and persist the effective MetaComm parameters into params.json via input_args.
    if input_args.algo in ('MetaComm', 'UCSMAPPO'):
        if getattr(input_args, 'metacomm_topology', 'learned') != 'learned':
            run_args.name += f'_CommTopo={input_args.metacomm_topology}'
        input_args.metacomm_lr_schedule = getattr(input_args, 'metacomm_lr_schedule', 'linear')
        input_args.metacomm_lr_final_scale = float(getattr(input_args, 'metacomm_lr_final_scale', 0.1))
        input_args.metacomm_lr_anneal_end_iter = int(getattr(input_args, 'metacomm_lr_anneal_end_iter', 0))
        input_args.metacomm_head_dim = int(getattr(input_args, 'metacomm_head_dim', 0))
        input_args.metacomm_inter_head_coef = float(getattr(input_args, 'metacomm_inter_head_coef', 0.0))
        run_args.name += (
            f'_LRSched={input_args.metacomm_lr_schedule}'
            f'_LREndScale={input_args.metacomm_lr_final_scale:g}'
        )
        if input_args.metacomm_lr_anneal_end_iter > 0:
            run_args.name += f'_LRAnnealIter={input_args.metacomm_lr_anneal_end_iter}'
        commformer_args = getattr(alg_args.agent_args, 'commformer_args', None)
        if commformer_args is not None:
            if hasattr(commformer_args, 'n_head'):
                if getattr(input_args, 'n_head', -1) != -1:
                    commformer_args.n_head = int(input_args.n_head)
                input_args.n_head = int(commformer_args.n_head)
            if input_args.metacomm_head_dim > 0 and hasattr(commformer_args, 'n_embd'):
                commformer_args.n_embd = int(commformer_args.n_head * input_args.metacomm_head_dim)
                run_args.name += f'_HeadDim={input_args.metacomm_head_dim}'
            if hasattr(commformer_args, 'inter_head_diversity_coef'):
                if input_args.metacomm_inter_head_coef > 0.0:
                    commformer_args.inter_head_diversity_coef = input_args.metacomm_inter_head_coef
                input_args.metacomm_inter_head_coef = float(commformer_args.inter_head_diversity_coef)
                if input_args.metacomm_inter_head_coef > 0.0:
                    run_args.name += f'_HeadDivCoef={input_args.metacomm_inter_head_coef:g}'
            if hasattr(commformer_args, 'n_embd'):
                input_args.metacomm_effective_n_embd = int(commformer_args.n_embd)
                input_args.metacomm_effective_head_dim = int(commformer_args.n_embd // commformer_args.n_head)
            
            if hasattr(commformer_args, 'sparsity'):
                if getattr(input_args, 'sparsity', -1.0) != -1.0:
                    commformer_args.sparsity = float(input_args.sparsity)
                    run_args.name += f'_Sparsity={commformer_args.sparsity}'
                input_args.sparsity = float(commformer_args.sparsity)

    if input_args.algo == 'UCSMAPPO':
        input_args.history_len_L = int(getattr(input_args, 'history_len_L', 20))
        input_args.gtrxl_mem_len = int(input_args.history_len_L)

    if input_args.env == 'UCS':
        run_args.name = f'{timenow}_{input_args.dataset}_{agent_fn.__name__}'
        if input_args.algo == 'UCSMAPPO':
            run_args.name += f'_HistL={int(getattr(input_args, "history_len_L", 20))}'
        run_args.name += f'_Seed={run_args.seed}'
        if getattr(input_args, 'use_intrinsic', False):
            run_args.name += f'_RNDCoef={input_args.intrinsic_coef:g}'
...
def record_input_args(input_args, env_args, output_dir):
    params = dict()
    if input_args.env == 'UCS':
        from envs.ucs_util.config_3d import Config
...
if __name__ == '__main__':
    ...
    elif input_args.algo == 'MetaComm':
        from algorithms.algo.agent.MetaComm import MetaCommAgent as agent_fn
    elif input_args.algo == 'CommG2ANet':
        from algorithms.algo.agent.CommG2ANet import CommG2ANetAgent as agent_fn
    elif input_args.algo == 'CommNet':
        from algorithms.algo.agent.CommNet import CommNetAgent as agent_fn
    elif input_args.algo == 'UCSMAPPO':
        from algorithms.algo.agent.UCSMAPPO import UCSMAPPOAgent as agent_fn

    if input_args.env == 'Mobile':
        from envs.env_mobile import EnvMobile
        env_fn_train, env_fn_test = EnvMobile, EnvMobile
    elif input_args.env == 'UCS':
        from envs.env_ucs import EnvUCS
