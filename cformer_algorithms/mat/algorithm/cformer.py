...
import math
import numpy as np
from envs.neighbor_graph import get_adj, get_grid_adj
...
try:
    ...
    try:
        ...
        # transformers >= 4.45 removed Conv1D entirely; redefine it
...
from cformer_algorithms.utils.util import check, init
from cformer_algorithms.utils.transformer_act import discrete_autoregreesive_act
from cformer_algorithms.utils.transformer_act import discrete_parallel_act
from cformer_algorithms.utils.transformer_act import continuous_autoregreesive_act
from cformer_algorithms.utils.transformer_act import continuous_parallel_act
...
class RelationMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, relation, attn_mask=None, need_weights=False, dec_agent=False):
        ...
        if dec_agent is True:
            ...
            # attn_mask: [tgt, src, bsz] → [tgt, src, bsz*num_heads] to match attn_weights
            if attn_mask.size(-1) != attn_weights.size(-1):
                attn_mask = attn_mask.repeat_interleave(self.num_heads, dim=-1)
            attn_weights.masked_fill_(
                attn_mask == 0,
                float('-inf')
            )
...
class Encoder(nn.Module):
    ...
    def forward(self, state, obs, relation, attn_mask, dec_agent, return_attention=False):
...
class CommFormer(nn.Module):
    ...
    def __init__(self, state_dim, obs_dim, action_dim, n_agent,
                 ...
                 no_relation_enhanced=False, tau=1.0,
                 fixed_edge_mode='learned', fixed_edge_seed=0):
        ...
        self.action_type = action_type
        self.device = device
        self.fixed_edge_mode = fixed_edge_mode
        self.fixed_edge_seed = fixed_edge_seed
        self.uses_fixed_topology = fixed_edge_mode != 'learned'
        ...
        self.edges = nn.Parameter(
            torch.ones(n_agent, n_agent),
            requires_grad=not self.uses_fixed_topology
        )
        self.edges_embed = nn.Embedding(2, n_embd)

        if self.uses_fixed_topology:
            self.register_buffer(
                "fixed_edges",
                self._build_fixed_edges(fixed_edge_mode, n_agent, fixed_edge_seed).to(
    ...
    def _build_fixed_edges(self, mode, n_agent, seed):
        if mode == 'full':
            return torch.as_tensor(get_adj(n_agent, fully_collect=True), dtype=torch.float32)
        if mode == 'fixed_grid':
            return torch.as_tensor(get_grid_adj(n_agent, seed=seed), dtype=torch.float32)
        raise ValueError(f'Unsupported fixed_edge_mode: {mode}')
    ...
    def edge_return(self, exact=False, topk=-1):
        ...
        edges = self.edges
        if exact is False:
            relations = gumbel_softmax_topk(edges, topk=self.topk, hard=True, dim=-1, tau=self.tau)
        else:
